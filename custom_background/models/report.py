...
class IrActionsReport(models.Model):
    ...
    def add_pdf_watermarks(self, custom_background_data, page):
        ...
        # Convert background data to base64 for b64decode. #T12134
        background_data_base64 = custom_background_data.to_base64()
        back_data = base64.b64decode(background_data_base64)
    ...
    def _get_background_per_report_company_language(self):
        ...
        # Deprecated _context attribute changed to self.env.context and added
        # fallback to self.env.company if background_company not in context. #T12134
        company = self.env.context.get("background_company") or self.env.company
    ...
    def get_bg_per_lang(self):
        ...
        """
        # Deprecated _context attribute changed to self.env.context and added fallback
        # to self.env.company if background_company not in context. #T12134
        company_background = (
            self.env.context.get("background_company") or self.env.company
        )
        lang_code = self.get_lang()
        ...
        if self.custom_report_type == "company" or not self.custom_report_type:
            # Added fallback if background_company not in context. #T12134
    ...
    def _dynamic_background_per_report(self, report, pdf_report_path):  # noqa: C901
        ...
        if (
            ...
            if (
                ...
                # Deprecated _context attribute changed to self.env.context and added
                # fallback to self.env.company if background_company not in
                # context. #T12134
                and (self.env.context.get("background_company") or self.env.company)
            ):
